...
handle_call({start_tcp_server,NewIpAddr,NewPort,NewMode}, _From, State) ->
    ...
    {Reply,NewState}=case TcpServers of
			 []-> % Only one Server
			     case NewMode of
				 sequence->
					 ...
					 {ok,SeqServer}->
						% NewState=State#state{tcp_servers=[{NewIpAddr,NewPort,NewMode,SeqServer}|TcpServers]},
					     {ok,State#state{tcp_servers=[{NewIpAddr,NewPort,NewMode,SeqServer}|TcpServers]}}
				     end;
				 parallell->
				...
				%	     NewState=State,
					 ...
					 {ok,ParServer}->
					     ...
					     {ok,State#state{tcp_servers=[{NewIpAddr,NewPort,NewMode,ParServer}|TcpServers]}}
				     end;
				 Err->
				     %NewState=State,
				     {{error,Err},State}
			 ...
			 [{_,_,_,_}]->
			     %NewState=State,
			     {{error,[already_started,NewIpAddr,NewPort,?MODULE,?LINE]},State}
		     end,
...
handle_call({stop_tcp_server,StopIpAddr,StopPort}, _From, State) ->
    ...
    {Reply,NewState}=case TcpServers of
			 []->
			    % NewState=State,
			     {{error,[not_started,StopIpAddr,StopPort,?MODULE,?LINE]},State};
			 [{StopIpAddr,StopPort,Mode,Server}]->
			    ...
			    % NewState=State#state{tcp_servers=lists:delete({StopIpAddr,StopPort,Mode,Server},TcpServers)},
			     {{ok,stopped},State#state{tcp_servers=lists:delete({StopIpAddr,StopPort,Mode,Server},TcpServers)}}
	  end,
    {reply, Reply, NewState};
