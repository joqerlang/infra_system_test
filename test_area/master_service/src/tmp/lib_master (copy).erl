...
init(CatalogInfo,NodesInfo)->
    ok=start_service("dns_service",".",node(),CatalogInfo,NodesInfo),
    ok. 
%% --------------------------------------------------------------------
%% Function: 
%% Description:
...
start_service(ServiceId,NodeDir,Node,CatalogInfo,NodesInfo)->
    ...
    {_NodeId,Node,IpAddr,Port}=lists:keyfind(Node,2,NodesInfo),

    ok=container:create(Node,NodeDir,
			[{{service,ServiceId},
			  {Source,Path}}
			]),
    dns:add(ServiceId,IpAddr,Port,Node).
%% --------------------------------------------------------------------
%% Function: 
%% Description:
%% Returns: non
...
service_node_info(Key,ServiceNodeInfo)->
    [{ServiceId,NodeId}||{ServiceId,NodeId}<-ServiceNodeInfo,ServiceId=:=Key].
...
%% --------------------------------------------------------------------
create_service_list()->
    AppList=master_service:apps(),
    create_service_list(AppList,[]).
create_service_list([],ServiceList)->
    ServiceList;
create_service_list([{ServiceId,_Num,[]}|T],Acc)->
    create_service_list(T,[{ServiceId,[]}|Acc]);
create_service_list([{ServiceId,_Num,Nodes}|T],Acc) ->
    L=[{ServiceId,Node}||Node<-Nodes],
    NewAcc=lists:append(Acc,L),
    create_service_list(T,NewAcc).
...
set_app_list(ServiceId,_IpAddr,_Port,Status)->
