...
start()->
    ...
    lib_service:start_tcp_server("joqhome.dynamic-dns.net",40000,parallell), 
    ok=application:start(dns_service), 
    dns_service:add("dns_service","joqhome.dynamic-dns.net",40000),
    ok=application:start(log_service), 
    dns_service:add("log_service","joqhome.dynamic-dns.net",40000),
    ok=application:start(master_service),
    dns_service:add("master_service","joqhome.dynamic-dns.net",40000),    
  %  check_nodes(),
    ok.
